...
# Generate two 200-config datasets in parallel: grid vs adaptive probe sampling.
# Usage:
#   bash 3_experiments/scripts/run_probe_mode_compare_200configs.sh
#
# Optional overrides (env vars):
#   MAX_CONFIGS=200 MAX_PROBES=128 GRID_RES=6
#   SH_MODE=cubemap CUBE_RES=32 NUM_FRAMES=32 SPP=512
#   OUT_ROOT=/path/to/output
#   PARALLEL=1  (default: 1)
#   MOGWAI_GPU_GRID=0 MOGWAI_GPU_ADAPTIVE=1 (optional GPU split)
...
OUT_ROOT="${OUT_ROOT:-$ROOT_DIR/1_data_generation/output/probe_mode_compare_200configs}"
...
run_job() {
  ...
  local args="$MOGWAI_ARGS"
  if [[ -n "$gpu_override" ]]; then
    args="--device-type $MOGWAI_DEVICE --gpu $gpu_override"
  fi
  ...
  "$FALCOR_MOGWAI_BIN" --headless --script "$ROOT_DIR/1_data_generation/falcor/generate_multilight_falcor.py" $args
}
...
if [[ "$PARALLEL" == "1" ]]; then
  ...
  run_job "grid" "$OUT_ROOT/grid/multilight" "$FALCOR_HOME_DIR/grid" "${MOGWAI_GPU_GRID:-}" &
  pid_grid=$!
  run_job "adaptive" "$OUT_ROOT/adaptive/multilight" "$FALCOR_HOME_DIR/adaptive" "${MOGWAI_GPU_ADAPTIVE:-}" &
  pid_adaptive=$!
  wait "$pid_grid"
  wait "$pid_adaptive"
else
