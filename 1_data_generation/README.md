...
## Falcor/Bistro 时序数据生成（推荐）

该流程使用 Falcor + Bistro Exterior 场景，在街道上放置三颗 **可见的发光球体**（红/绿/蓝），生成 **600 帧**时序光照探针数据。  
为提升速度，默认使用 **cubemap 模式**（比方向采样快 1~2 个数量级）。

### 0) 资源准备

确保 Bistro 资产已解压到：
```
1_data_generation/scenes/Bistro_v5_2/
```
...
### 2.1) 深度引导探针采样（可选，推荐）
该步骤会通过 GBufferRT 输出表面点云，并按“发光球距离”加权采样探针位置。
...
### 关键说明
- **SPP 限制**：Falcor PathTracer 每帧 SPP 最多 16。脚本会自动用 `accum-frames` 累积达到你设定的总 SPP。
- **探针分布**：`adaptive` 模式 = 70% 均匀 + 30% 近表面采样（当前为包围盒近表面）。
- **输出数据**：`parametric_tensor.npz` 内包含 `tensor [P, M, 27]`、`probe_positions`、`light_configs`、`metadata`。
